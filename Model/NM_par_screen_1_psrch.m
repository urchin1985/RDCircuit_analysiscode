...
%     prm.ki = 5*rand(1,2); % vary between .25-2.5 %[.5 .5]
%     prm.ni = 5*rand(1,2); % vary between 1-4 % [1 3]
%     prm.k = 5*rand(2,2); % vary between .2-3 % [0 1;.3 .5]
%     prm.n = 1.7+3.3*rand(2,2); % vary between 1.7-5 % [0 4;3 2]
%     prm.b = 5*rand(1,2); % baseline activity % vary between .5-2 % [.5 .2]+1
%     prm.a = 5*rand(1,2); % gain on chemosensory input % vary between .5-5 % [1.3 1.8]
%     a1o = prm.a(1);
%     prm.beta = 1+4*rand(1,2); % gain on mutual inhibition % vary between 1-5 % [3.25 2]

    prm.ki = 5*rand(1,2); % vary between .25-2.5 %[.5 .5]
    prm.ni = 4*rand(1,2); % vary between 1-4 % [1 3]
    prm.k = 5*rand(1,2); % vary between .2-3 % [0 1;.3 .5]
    prm.n = 1.7+3.3*rand(1,2); % vary between 1.7-5 % [0 4;3 2]
%     prm.b = .5+1.5*rand(1,2); % baseline activity % vary between .5-2 % [.5 .2]+1
    prm.a = .5+4.5*rand(1,2); % gain on chemosensory input % vary between .5-5 % [1.3 1.8]
    a1o = prm.a(1);
    prm.beta = 1+4*rand(1,2); % gain on mutual inhibition % vary between 1-5 % [3.25 2]
