% clear;
% grab_prms
%% simulate chemotaxis to dense food
prm.a(1) = 1.3; %prm.a(2)=2.1;
% prm.r(2) = .3;
prm.k(4)= 1.35; %prm.n(4)=2;

figure(29);clf;
N = 300; lend = [];
XS0=[0 1.1]; % initial condition
pbl = 0;

for ci = 1:1500
    % initialize params
    clear ha RD inc.i
    ha(1) = 2*(rand-.5)*pi; RD(1) = 1; inp.i = 0;
    aem = 5.2; hp1 = .1; rsp = .5; % roaming speed
    wsp = rsp; inp.fc = prm.fc;
    rmg = 1; bx = 50;
    nth = 1;gd = 2.5; % sensory gain along gradient dir
    han = normrnd(0,aem,[1 N]); han = smooth(han,51); % heading angle noise
    % note: 1) duration of dwelling set to 5 tps; 2) keep speed binary for
    % simplicity
    %%
    XS = XS0;
    loc0 = [0 0];
    loc = loc0;
    Xt=XS;
    for ti=2:N
        %%
        % execute movement
        if RD(ti-1) % if roaming as of last time point
            loc(ti,:) = loc(ti-1,:)+[cos(ha(ti-1)) sin(ha(ti-1))]*wsp;
            if XS(ti-1,1)>nth; rmg = rmg-1; end
        else
            loc(ti,:) = loc(ti-1,:);
        end
        
        % calculate sensory input based on recent movement
        inp.i(ti) = max(0,gd*(loc(ti,1)-loc(ti-1,1)));
        
        % compute and update circuit state
        run_cmod2
        Xt=max(0,(Xt+(dX*prm.dt)));
        XS=[XS;Xt];
        
        %%%% determine which state animal will be in-->update every 10 tps
        %%%% 
        if mod(ti,2)==0
            if Xt(1)>nth
                wsp = 0; RD(ti) = 0; % dwell
            else
                wsp = rsp; RD(ti) = 1; % roam
            end
            
            if RD(ti-1) == 0
                RD(ti) = 1; 
            wsp = rsp;
            XS(ti,1) = 0; Xt(1) = 0;
            end

            % compute new heading direction
            if RD(ti) % movement only occurs during roaming
                % first determine direction of motion
                if RD(ti-1)==0 % if roaming onset, randomly choose direciton
                    ha(ti) = 2*(rand-.5)*pi;
                else
                    dha = hp1*(-abs(ha(ti-1))); % klinotaxis
                    ha(ti) = (abs(ha(ti-1))+dha*prm.dt)*sign(ha(ti-1))+han(ti)*prm.dt;
                end
            else
                ha(ti) = nan;
            end
                    if Xt(1)>nth&RD(ti)>0; wt; end
        else
            ha(ti) = ha(ti-1);
            RD(ti) = RD(ti-1);
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end
    
    % record endpoint
    lend(ci,:) = loc(end,:);
    
    if pbl
        % plot ctx behavior
        figure(29);hold all
        subplot(5,1,1:3); hold all
        plot(loc(:,1),loc(:,2),'.-')
        plot(loc(1,1),loc(1,2),'ro')
        dwi = RD==0;
        plot(loc(dwi,1),loc(dwi,2),'r.','markersize',32)
        subplot 514; hold all
        plot(XS(:,1));plot(XS(:,2))
        subplot 515; cla;hold all
        yyaxis left;plot(inp.i)
        yyaxis right; plot(RD)
    end
end
figure(32);hist(lend(:,1))
[mean(lend(:,1)) median(lend(:,1))]