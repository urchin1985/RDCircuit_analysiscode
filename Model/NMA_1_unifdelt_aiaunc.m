...
idn = 1300; icf = 1;
fid = 0; dq = 0; nl = .25;
XS0=[6 0]; rN = 200; rmpc = cell(1,2);
%%
iplst = 0.1:.1:2.5; b1lst = -3:.2:2;
...
for ii = 1:(length(iplst))
    ...
    for bi = 1:(length(b1lst))
        ...
        cmap = cmap_gen_flx({[1 0 0],[0 0 1]},[100 40]);
        colormap(cmap)
        caxis([-1 .4])
        drawnow limitrate
    end
...
surf(zeros(size(rdtmat)),rdtmat,'linestyle','none','facecolor','interp')
...
set(gca,'xlim',1+[2.5 length(b1lst)-2.5],'ylim',[6.3 length(iplst)-8.7],...
    'xtick',3.5:10:31,'ytick',6.5:4.5:30,'xticklabel','','yticklabel','',...
    'tickdir','out','ticklength',.035*[1 1],'layer','top')
set(gcf,'outerposition',[20 380 270 336])
%% saving plot and data
setsavpath
if svon
    savname = 'NMA_deltrm_aia_v3';
    saveas(gcf,[savpath2 savname '.tif'])
    saveas(gcf,[savpath2 savname '.fig'])
    saveas(gcf,[savpath2 savname '.eps'],'epsc')
    
    save([savpath savname '.mat'],'iplst','b1lst','prm','icf','nl','rdtmat')
end