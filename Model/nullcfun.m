function [nco,uo] = nullcfun(xr1,xr2,inc,prm,fid,dq)
% plot nullclines
x1 = xr1(1):inc(1):xr1(2);
x2 = xr2(1):inc(2):xr2(2);
xo = modfun(x1,x2,prm);
[ux1,ux2] = meshgrid(x1,x2);
uo = modfun(ux1,ux2,prm);
...
if fid>=0
    ...
    hold all
    % plot vector field
    surf(ux1-inc(1),ux2-inc(2),zeros(size(ux1)),up,'linestyle','none')
    caxis([0 1])
    if dq
        quiver(ux1,ux2,uo.dx1,uo.dx2,'b','LineWidth',1);
    end
%     plot([xr1(1) xr1(2);0 0]',[0 0;xr2(1) xr2(2)]','k:','linewidth',1.5)
    
    plot(xo.nx1,x2,'r','LineWidth',1);
    hold on; plot(x1,xo.nx2,'g','LineWidth',1)
