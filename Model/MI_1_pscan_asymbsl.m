...
[xb,yb] = meshgrid(0:0.15:3.5,-1:0.15:2);
rmt = nan(size(xb));
nmt = rmt;
...
for xi = 1:size(xb,2)
    ...
    for yi = 1:size(xb,1)
        prm.b(1) = xb(yi,xi);prm.b(2) = yb(yi,xi);
        ...
        rcr = (nanmax(rmy)-rmy(1))*(nanmax(rmy)-rmy(end));
        rpn = (nanmax(rmn)-rmn(1))*(nanmax(rmn)-rmn(end));
...
rnmt = rmt.*(nmt>0);
figure(fiid);clf;hold all
surface(xb,yb,rmt,rmt) %,'facecolor','interp'
xlabel('bs1');ylabel('bs2')
%
setsavpath
