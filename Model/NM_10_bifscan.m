...
plot(fp.x','k.','markersize',12)

fd = abs(diff(fp.x)); fdb = fd>1.5;
fpp = regionprops(fdb,'PixelIdxList');
fpb = fpp(1).PixelIdxList([1 end]);
plot(fpb(1)*ones(1,2),fp.x(:,fpb(1)),'k:','linewidth',1)
plot(fpb(2)*ones(1,2),fp.x(:,fpb(2)),'k:','linewidth',1)
...
for ii = 1:length(ilst)
    prm.i = ilst(ii);
    inp.i = [prm.i*ones(1,idn)];
    [nco,uo] = nullcfun_nm(xr1,xr2,inc,prm,2,dq);    
...
figure(61);hold all
subplot 211; hold all
plot(fp.x','b.','markersize',12)

fd = abs(diff(fp.x)); fdb = fd>1.5;
fpp = regionprops(fdb,'PixelIdxList');
fpb = fpp(1).PixelIdxList([1 end]);
plot(fpb(1)*ones(1,2),fp.x(:,fpb(1)),'b:','linewidth',1)
plot(fpb(2)*ones(1,2),fp.x(:,fpb(2)),'b:','linewidth',1)
set(gca,'xticklabel','')
ylabel('Steady state X1 value')
subplot 212; hold all
...
ylabel('Steady state X2 value'); xlabel('Input level (a.u.)')
