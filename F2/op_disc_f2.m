...
for oi = 1:length(ontim)
    ...
    % sort by state before stim
    %     otpre = nanmean(otmat(:,1:30),2); % speed ~3s before stim onset
    %     otpos = nanmean(otmat(:,30:60),2); % speed ~6-9s after
    %     otlat = nanmean(otmat(:,150:180),2);
    %     oki2 = find(~isnan(otpre));
    %     otpre = otpre(oki2); otpos = otpos(oki2); otlat = oplat(oki2);
