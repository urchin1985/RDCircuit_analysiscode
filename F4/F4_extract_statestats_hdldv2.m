...
% pclr = {.6*[1 1 1],[.55 .86 .55],.6*[1 1 1],[.55 .86 .55]}; 
...
%% save plots and p vals
if svon
    figure(fid)
    savname = 'WT_AIAunc_hdrmfrc_f';
    saveas(gcf,[savpath2 savname '.tif'])
    saveas(gcf,[savpath2 savname '.fig'])
    saveas(gcf,[savpath2 savname '.eps'],'epsc')
    
    figure(fid+1)
    savname = 'WT_AIAunc_ldhd_dwdur_f';
    saveas(gcf,[savpath2 savname '.tif'])
    saveas(gcf,[savpath2 savname '.fig'])
    saveas(gcf,[savpath2 savname '.eps'],'epsc')
    
    figure(fid+2)
    savname = 'WT_AIAunc_ldhd_rmdur_f';
    saveas(gcf,[savpath2 savname '.tif'])
    saveas(gcf,[savpath2 savname '.fig'])
    saveas(gcf,[savpath2 savname '.eps'],'epsc')
end