...
varout = nsmtrigger_on(NTR,cdata(:,1),vdata,fidata,nsm_gmfit,tpre,tpost);
N_on = varout.tdon;
...
wspdat = abs(N_on(1).vals); 
spout = cal_matmean(wspdat,1,1);
mf = 3;
...
varout = optotrigger_on(NTR,cdata(:,1),vdata,fidata,tpre,tpost);
N_on = varout.tdon;
...
% vm = nanmean(V_on(1).vals);
spdat = abs(N_on(1).vals); 
spout = cal_matmean(spdat,1,1);
mf = 11;
