...
if svon
    figure(flst)
    savname = [savpath2 gtype '_' cnmvec{pd(1)} '-' cnmvec{pd(3)} '_xcorr_exts'];
    saveas(gcf,[savname '.tif'])
    saveas(gcf,[savname '.fig'])
    saveas(gcf,[savname '.eps'],'epsc')
    
    figure(flst+1)
    savname = [savpath2 gtype '_' cnmvec{pd(4)} '-' cnmvec{pd(3)} '_xcorr_exts'];
    saveas(gcf,[savname '.tif'])
    saveas(gcf,[savname '.fig'])
    saveas(gcf,[savname '.eps'],'epsc')
    
    figure(flst+2)
    savname = [savpath2 gtype '_' cnmvec{pd(end)} '-' cnmvec{pd(3)} '_xcorr_exts'];
    saveas(gcf,[savname '.tif'])
    saveas(gcf,[savname '.fig'])
    saveas(gcf,[savname '.eps'],'epsc')
    
    save([savpath gtype '_xcorr_pnonts2.mat'],'T_on','Tm','strd','wl','xcfset',...
        'pmt','fdrp','pd')
end