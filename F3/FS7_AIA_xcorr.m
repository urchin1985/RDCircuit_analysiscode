...
if svon
figure(flst)
savname = [savpath2 gtype '_' cnmvec{pd(1)} '-' cnmvec{pd(3)} '_xcorr_ex'];
saveas(gcf,[savname '.tif'])
saveas(gcf,[savname '.fig'])
saveas(gcf,[savname '.eps'],'epsc')

figure(flst+1)
savname = [savpath2 gtype '_' cnmvec{pd(2)} '-' cnmvec{pd(3)} '_xcorr_ex'];
saveas(gcf,[savname '.tif'])
saveas(gcf,[savname '.fig'])
saveas(gcf,[savname '.eps'],'epsc')

figure(flst+2)
savname = [savpath2 gtype '_' cnmvec{pd(end)} '-' cnmvec{pd(3)} '_xcorr_ex'];
saveas(gcf,[savname '.tif'])
saveas(gcf,[savname '.fig'])
saveas(gcf,[savname '.eps'],'epsc')

save([savpath gtype '_xcorr_pnonex.mat'],'T_on','Tm','strd','wl','xcfset')
end