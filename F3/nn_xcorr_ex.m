...
for tri = 1:tn
    for wi = 1:wn
        ...
        tx = tx-prctile(tx,5);
        if c1==1
            if (wi>2&&max(tx)>.3)||(wi<3&&max(tx)>.015&&max(tx)<.1)%wi>=wpr||(max(tx)>.015&&max(tx)<.1)
...
for wi = 1:wn
    if ~isempty(xcmat{wi})
        ...
        text(xmi-1-cw,wout.se(xmi)+wout.mean(xmi)+.05,'*','fontsize',18,'color','r')
        plotstandard
