...
plot(clid)
xlim([0 length(vdata)+5]); ylim([-.5 3])
ylabel('Normalized axial velocity')
% [x,y] = ginput(1);
end
%% save GM model
% save([savpath gtype '_NSM_gmfit_e.mat'],'Xd','Fdx','gmfit','AIC','BIC') % ,'-append'
save([savpath gtype '_AIA_gmfit.mat'],'Xd','Fdx','clx','gmfit')
ca_gmfit = gmfit.mu;

figure(18)
savname = [gtype '_AIAclust_gmfit'];
...
%
savname = [gtype '_AIAclust_modsel'];
saveas(gcf,[savpath2 savname '.tif'])
saveas(gcf,[savpath2 savname '.fig'])
saveas(gcf,[savpath2 savname '.eps'],'epsc')
