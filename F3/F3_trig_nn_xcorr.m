...
if svon
figure(flst(1))
savname = [savpath2 gtype '_RM-' cnmvec{pd(3)} '_xcorr_periNSMon'];
saveas(gcf,[savname '.tif'])
saveas(gcf,[savname '.fig'])
saveas(gcf,[savname '.eps'],'epsc')

figure(flst(2))
savname = [savpath2 gtype '_' cnmvec{pd(4)} '-' cnmvec{pd(3)} '_xcorr_periNSMon'];
saveas(gcf,[savname '.tif'])
saveas(gcf,[savname '.fig'])
saveas(gcf,[savname '.eps'],'epsc')

save([savpath gtype '_xcorr_periNSMon.mat'],'T_on','Tm','strd','wl','xcfset')
end